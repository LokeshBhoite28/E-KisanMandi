...
public class WebSecurityConfig extends WebSecurityConfigurerAdapter {
	...
	protected void configure(HttpSecurity http) throws Exception {
		...
		and().
		authorizeRequests()	
		.antMatchers("/api/user/**").hasAnyRole("ADMIN","FARMER","TRADER")
		.antMatchers("/api/farmer/**").hasRole("FARMER")
		.antMatchers("/api/trader/**").hasRole("TRADER")
		.antMatchers("/api/crops/**").hasAnyRole("FARMER","TRADER")
		.antMatchers("/api/auth/**","/swagger*/**", "/v3/api-docs/**").permitAll() //enabling global access to all urls with /api/auth 
